			...
			for(int j = 0; j < vertices.size()-1; j++)
			{
				...
				for(int k = 0; k < vertices.size()-1; k++)
				{
					...
					if(k != j)
					{
						...
						// now check the coordinates for w
		...
		for(int j = 0; j < vertices.size()-1; j++)
		{
			...
			for(int k = 0; k < vertices.size()-1; k++)
			{
				...
				if(k != j)
				{
					...
					// Diese Idee hatten wir vorher auch umgesetzt, nun mit einer Randomnumber zwischen 0 und 30, da haben ich zur besseren Trennung das Intervall verbessert
					if(v.getX() >= frameWidth/2)
					{
						int randomX = rand2.nextInt(60);
					...
					// further separation of the vertices, the +20 results in the radius of the drawn vertices, such that the vertices shouldn't overlap too often
					if(v.getX() >= w.getX())
					{
						v.setX(v.getX() + 20); 
					}
